...
export class WasabiTable {
    constructor(wasmTable, config, canvas, recordsSource = null) {
        ...
        this.boundHandleContextMenuOutsidePointer = (event) => this.handleContextMenuOutsidePointer(event);
        this.boundHandleContextMenuKeydown = (event) => this.handleContextMenuKeydown(event);
        /**
    ...
    static installGlobalEditingHandlers() {
        if (WasabiTable.globalEditingHandlersInstalled)
            return;
        const win = window;
        win.handleEditingEnter = () => {
            var _a;
            (_a = WasabiTable.activeEditingTable) === null || _a === void 0 ? void 0 : _a.handleEditingEnter();
        };
        win.handleEditingTab = () => {
            var _a;
            (_a = WasabiTable.activeEditingTable) === null || _a === void 0 ? void 0 : _a.handleEditingTab();
        };
        win.handleEditingEscape = () => {
            var _a;
            (_a = WasabiTable.activeEditingTable) === null || _a === void 0 ? void 0 : _a.handleEditingEscape();
        };
    ...
    setupScrollbarEvents() {
        ...
        if (!this.horizontalScrollbar || !this.verticalScrollbar ||
            ...
            return;
        const signal = (_a = this.eventAbortController) === null || _a === void 0 ? void 0 : _a.signal;
        // 水平スクロールバーのクリックイベント
     ...
     * スクロールサムのドラッグ機能を設定
     */
    setupThumbDrag(thumb, direction, signal) {
        let isDragging = false;
        let startPos = 0;
        let startScroll = 0;
    ...
    setContextMenuOptions(options) {
        this.config.contextMenu = options !== null && options !== void 0 ? options : undefined;
    ...
    handleCanvasContextMenu(event) {
        var _a, _b;
        if (((_a = this.config.contextMenu) === null || _a === void 0 ? void 0 : _a.enabled) === false)
            return;
        const rect = this.canvas.getBoundingClientRect();
        const x = event.clientX - rect.left;
        const y = event.clientY - rect.top;
        ...
        this.hideMenuFieldSelectBox();
        (_b = this.headerDialogController) === null || _b === void 0 ? void 0 : _b.hideAll();
        const [row, col] = cellPos.split(':').map(Number);
    ...
    finishEditingIfNeededBeforeContextMenu() {
        if (this.isEditing()) {
            this.finishEditing();
            this.syncActiveFormulaBarValue();
        }
    }
    selectionContainsCell(selection, row, col) {
    ...
    createContextMenuContext(row, col, selection, event) {
        var _a;
        return {
            table: this,
            cell: {
                row,
                col,
                value: (_a = this.getCellValue(row, col)) !== null && _a !== void 0 ? _a : '',
    ...
    getContextMenuActions(context) {
        var _a, _b, _c;
        const configuredBuiltIns = (_a = this.config.contextMenu) === null || _a === void 0 ? void 0 : _a.builtInActions;
        const builtInIds = configuredBuiltIns === false
            ? []
            : configuredBuiltIns !== null && configuredBuiltIns !== void 0 ? configuredBuiltIns : DEFAULT_CONTEXT_MENU_ACTIONS;
        const builtInActions = builtInIds.map((id) => this.createBuiltInContextMenuAction(id));
        const customActions = (_c = (_b = this.config.contextMenu) === null || _b === void 0 ? void 0 : _b.actions) !== null && _c !== void 0 ? _c : [];
    ...
    handleContextMenuOutsidePointer(event) {
        if (!this.contextMenuElement)
            return;
     ...
     * クリップボードからペースト
     */
    pasteFromClipboard(tsvData, options = {}) {
        var _a;
        this.ensureInitialized();
    ...
    preparePasteRows(rows, options) {
        ...
        for (let col = 0; col < colCount; col += 1) {
            ...
            for (let row = 0; row < rowCount; row += 1) {
                targetRow.push((_b = (_a = rows[row]) === null || _a === void 0 ? void 0 : _a[col]) !== null && _b !== void 0 ? _b : '');
            }
    ...
    async pasteClipboardToSelection(options = {}) {
        try {
            let pasteData = '';
    ...
    getHeaderPosition(columnIndex) {
        ...
        const width = (_a = layout === null || layout === void 0 ? void 0 : layout.width) !== null && _a !== void 0 ? _a : this.getColumnWidthAt(columnIndex);
        return {
            x: canvasRect.left + ((_b = layout === null || layout === void 0 ? void 0 : layout.startX) !== null && _b !== void 0 ? _b : config.row_header_width),
            y: canvasRect.top,
            width,
    ...
    getFrozenColumnCount() {
        var _a;
        return Math.min((_a = this.config.freeze_cols) !== null && _a !== void 0 ? _a : 0, this.config.col_count);
    }
...
WasabiTable.activeEditingTable = null;
WasabiTable.globalEditingHandlersInstalled = false;
